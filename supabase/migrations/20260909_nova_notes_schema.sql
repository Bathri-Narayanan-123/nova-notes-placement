...
-- Migration: 20260909_nova_notes_schema.sql
-- ==============================================================================
...
-- Helper to check if current authenticated user is an administrator
...
-- Profiles: Students manage own profile. Admin can view and inspect all profiles.
...
-- Questions & Options: Read by authenticated users, Managed by Admin
...
CREATE POLICY "Assessment questions read" ON public.assessment_questions FOR SELECT USING (true);
CREATE POLICY "Assessment questions admin manage" ON public.assessment_questions FOR ALL USING (public.is_admin());

-- Attempts & Answers: Students read/write own; Admin can read all
CREATE POLICY "Attempts student manage" ON public.assessment_attempts
    FOR ALL USING (auth.uid() = student_id OR public.is_admin());

CREATE POLICY "Answers student manage" ON public.assessment_answers
    FOR ALL USING (
        EXISTS (
...
-- Practice Sessions
CREATE POLICY "Practice sessions manage" ON public.practice_sessions
    FOR ALL USING (auth.uid() = student_id OR public.is_admin());
CREATE POLICY "Practice answers manage" ON public.practice_answers
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM public.practice_sessions s 
            WHERE s.id = session_id AND (s.student_id = auth.uid() OR public.is_admin())
        )
    );

-- Interviews & Evaluations
CREATE POLICY "Interviews manage" ON public.interviews
    FOR ALL USING (auth.uid() = student_id OR public.is_admin());
...
CREATE POLICY "Interview evaluations manage" ON public.interview_evaluations
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM public.interviews i 
            WHERE i.id = interview_id AND (i.student_id = auth.uid() OR public.is_admin())
        )
    );

-- Performance & Scores
CREATE POLICY "Skill performance manage" ON public.skill_performance
    FOR ALL USING (auth.uid() = student_id OR public.is_admin());
CREATE POLICY "Readiness scores manage" ON public.readiness_scores
    FOR ALL USING (auth.uid() = student_id OR public.is_admin());

-- ==============================================================================
-- 5. TRIGGERS: AUTOMATIC PROFILE CREATION ON USER SIGNUP
-- ==============================================================================
...
-- ==============================================================================
-- 6. SEED DATA (Placement Roles & Configurations)
-- ==============================================================================

INSERT INTO public.roles (id, title, description, skills, icon, color) VALUES
('role-python', 'Python Developer', 'Specialized in backend APIs, scripting, data structures, OOP architecture, and database persistence.', ARRAY['Python', 'OOP', 'DSA', 'SQL', 'FastAPI', 'Algorithms'], 'Code', 'blue')
ON CONFLICT (id) DO NOTHING;

INSERT INTO public.roles (id, title, description, skills, icon, color) VALUES
('role-data-analyst', 'Data Analyst', 'Data wrangling, SQL aggregations, exploratory statistics, dashboarding, and actionable business metrics.', ARRAY['SQL', 'Python', 'Pandas', 'Tableau', 'Statistics', 'PowerBI'], 'BarChart3', 'emerald')
ON CONFLICT (id) DO NOTHING;

INSERT INTO public.roles (id, title, description, skills, icon, color) VALUES
('role-fullstack', 'Full Stack Developer', 'End-to-end web engineering, React UI, Node/Express backends, relational databases, and RESTful architectures.', ARRAY['JavaScript', 'React', 'Node.js', 'PostgreSQL', 'TypeScript', 'System Design'], 'Globe', 'indigo')
ON CONFLICT (id) DO NOTHING;

INSERT INTO public.roles (id, title, description, skills, icon, color) VALUES
('role-ml-engineer', 'Machine Learning Engineer', 'Predictive modeling, deep learning architectures, feature engineering, and MLOps deployment pipelines.', ARRAY['Python', 'PyTorch', 'Scikit-Learn', 'Math/Linear Algebra', 'MLOps'], 'Cpu', 'purple')
ON CONFLICT (id) DO NOTHING;

INSERT INTO public.roles (id, title, description, skills, icon, color) VALUES
('role-data-engineer', 'Data Engineer', 'Scalable distributed ETL pipelines, Apache Spark, data warehousing, Kafka streaming, and schema modeling.', ARRAY['Python', 'SQL', 'Spark', 'Airflow', 'Data Modeling', 'Cloud Warehouses'], 'Layers', 'amber')
ON CONFLICT (id) DO NOTHING;

INSERT INTO public.roles (id, title, description, skills, icon, color) VALUES
('role-ai-engineer', 'AI Engineer', 'LLM orchestration, agentic architectures, RAG pipelines, prompt evaluation, and vector retrieval.', ARRAY['Python', 'Gemini API', 'LangChain', 'Vector DBs', 'Embeddings', 'Fine-Tuning'], 'Sparkles', 'rose')
ON CONFLICT (id) DO NOTHING;

-- ==============================================================================
-- 7. PROMOTING PROJECT OWNER TO ADMIN (Single Admin Mandate)
-- Run this in the Supabase SQL editor to grant full administrator status:
